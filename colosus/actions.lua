    ...
    local sonarAttack = {
        ...
        execute = function(playerData, creatureData)
            ...
            local damage = math.max(1, math.ceil(rawDamage * 0.3))
                ...
                -- 4. Apresentar resultado como print
                print(string.format("%s usou um sonar %s e deu %d pontos de dano", creatureData.name, playerData.name, damage))
                local healthRate = math.floor((playerData.health / playerData.maxHealth) * 10)
                print(string.format("%s: %s", playerData.name, utils.getProgressBar(healthRate)))
            end
        }
        ...
        local waitAction = {
            ...
            -- 4. Apresentar resultado como print
            print(string.format("%s decidiou aguardar e não fez nada nesse turno", creatureData.name))
        end
    }
